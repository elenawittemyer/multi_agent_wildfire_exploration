...
size = 100
rand_gaussian = gaussian(size, 50, 50, 20)
meas_reduction = gaussian_measurement(size, 200, 180, 0)
map = rand_gaussian + meas_reduction
map[map<0]=0

robot_model     = SingleIntegrator(2)
target_distr    = TargetDistribution(map)
basis           = BasisFunc(n_basis=[8,8])
args = {
    'x0' : np.array([[2.0,3.25], [0., 0.]]),
    'xf' : np.array([[1.75, -0.75], [0., 0.]]),
    'phik' : get_phik(target_distr.evals, basis),
    'wrksp_bnds' : np.array([[0.,3.5],[-1.,3.5]])
}
traj_opt = MAErgodicTrajectoryOpt(robot_model, basis=basis, time_horizon=250, args=args)
print('solving traj')
